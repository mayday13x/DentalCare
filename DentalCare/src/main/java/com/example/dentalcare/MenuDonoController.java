...
public class MenuDonoController {
    ...
    public void menuCriarServico(ActionEvent event){
        try {
            Parent root = FXMLLoader.load(getClass().getResource("criarServico.fxml"));
            Scene regCena = new Scene(root);
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.setScene(regCena);
            stage.setTitle("Criar Servico");
            stage.show();
        }catch (IOException ex){
            System.out.println("Erro ao tentar acessar menu criar servico : " + ex.getMessage());
            ex.printStackTrace();
        }
    }
